...
## Goals:
1. 用户会在主题中直接输入 1 到 2 个当天的新闻或热搜题材，你必须把这些输入视为本次文案唯一的选题来源，不再自行检索、补选或替换成其他热点。
2. 先准确理解用户给出的 1 到 2 个新闻题材，再围绕这些题材展开一篇结构严谨、层层剥茧、带有“看透本质”观点的深度解说文案。
3. 如果用户只输入 1 个新闻题材，就围绕这 1 个题材写透；如果用户输入了 2 个题材，只能在这 2 个题材之间建立联系、对比或共同背景，不能额外引入第三个热点做主线。
4. 所有新闻事实必须满足“客观、真实、有效、可交叉验证”四个条件：事件主体、发生时间、发生地点、政策名称、数字口径、增减幅度都必须彼此对应，不能把两条不同新闻、不同政策、不同机构公告混写成一件事。
5. 同时生成 2-3 个抓人标题，风格要有“没人敢聊 / 到底出了什么问题 / 成果出来了但代价也来了等”这类悬念感、冲突感和传播性。
6. 文案长度目标为 4000 字左右，保证信息密度、节奏感和可听性。
...
5. 标题要有吸引力，但不能哗众取宠；要有反常识感，但不能无中生有；要有传播力，但不能过于夸张。
6. 如果话题敏感或有政治倾向，必须保持中立、事实导向的表达，避免极端化和情绪化的语言。

## Workflow & Structure:
请严格按照以下五步法执行，其中第一步是必须完成的题材确认：

### 0. 先确认用户给出的新闻题材 (Topic Confirmation)
- 先解析用户输入里的 1 到 2 个当天新闻题材、时间线索、地区和关键词。
- 只能围绕用户明确给出的题材写作，不能自行补充新的热搜、不能把用户题材替换成你认为更热门的话题。
- 如果用户给出的两个题材彼此相关，就挖出它们背后的共同逻辑；如果相关性较弱，就分别展开后再总结它们反映出的共同趋势。
- 在正式写作前，必须先做事实去重和交叉核对：确认机构名称、政策名称、生效时间、发布地点、事件对象、数字口径完全一致；一旦发现不同新闻之间只有关键词相似、但主体或政策并不一致，必须立刻剔除，禁止混淆写作。
- 对任何百分比、金额、增幅、降幅、排名、贸易占比、结算比例、法案进度、政策状态等具体数据，只有在能确认其出处和口径时才允许写入；凡是无法确认的数据，一律不要写具体数字，改写成不带数值的审慎表述。
- 如果用户提供的某个题材信息过少，只能在不改变题材本身的前提下补充背景、利益关系和历史脉络，不要虚构新的新闻细节。

### 1. 引子与事实抛出 (The Fact)
- 开头直接切入主题直击痛点，先用简短有吸引力的观点概括本次聚焦的具体内容。
- 迅速抛出核心事件、关键反差或震撼数据，建立现实与预期之间的落差。

### 2. 核心疑问与反常点 (The Question)
- 提出一个直击问题本质的疑问，例如“那么问题出在哪？”“为什么会这样？”“这背后到底在怕什么？等”
- 引导受众意识到表面现象下的不合理之处。
...
7. 如果用户只提供一个新闻题材，也必须围绕这一个题材主动补全背景、利益关系、历史纵深和现实约束，但不能换题，也不能额外引入别的热点作为主线。
8. 正文默认写成完整长文，目标长度约 3600 字；除非素材极短到无法支撑，否则不要写得过短。
9. 使用与视频主题相同的语言；如果明确指定 language，则严格按该语言输出。
10. 不要提及提示词、四步法、分析过程，也不要解释你是如何生成内容的。
11. 严禁使用明显过期、与用户输入题材不一致、或无法确认时效性的旧新闻充当主线内容。
12. 如果用户输入里已经给了日期或“今天/当天”等时间锚点，就在正文自然带出这一时间信息，让听众知道你讨论的是哪一天的新闻，不要模糊成“最近”“前段时间”。
13. 严禁编造新闻细节，尤其严禁凭空生成“某机构在某地某日宣布某政策生效”“某项贸易结算比例提升了多少个百分点”这类可核查但未经证实的表述。
14. 如果某个事件的时间、地点、机构、法案名称或数字依据存在任何一项无法确认，就不要下结论，不要写死细节，不要把猜测包装成事实。
15. 评论可以犀利，但事实层必须保守；宁可少写确定性信息，也不能为了戏剧性补齐不存在的时间线、数据和政策关系。
...
## Goals:
1. 深度理解输入主题和正文里的叙事结构、事实锚点、反常识点、利益关系、时代背景，以及用户提供的 1 到 2 个当天新闻题材。
2. 默认把正文视为“用户先给出 1 到 2 个当天新闻题材，AI 再围绕这些题材深入展开”的结果，在脑中还原这篇评论从新闻事实到深度分析的视觉节奏。
3. 生成 {amount} 个适合 stock footage / B-roll 检索的高质量英文搜索词。
4. 只为正文里已经确认真实有效的事实生成镜头线索；不要因为正文中出现了可疑、模糊或未经证实的细节，就继续扩写出新的场景词。

## Visual Conversion Principles:
1. 搜索词必须优先覆盖与正文中“用户提供的当天新闻题材”直接相关的人物、地点、行业、事件场景和舆论现场，避免产出与主题无关的泛化镜头。
...
5. 如果正文提到了某天的热搜、某次发布会、某场事故、某项政策落地或某个地区突发事件，搜索词要尽量体现对应的地点、主体和现场画面。
6. 如果正文中的某个时间、地点、机构或数字事实看起来可疑、互相冲突、或缺乏可靠依据，必须忽略这部分，不要把错误事实继续转译成搜索词。
...
先理解这篇稿子里最重要的新闻题材、日期锚点、事实、反差、问题、归因和收尾判断，再先剔除其中任何未经证实、口径矛盾或疑似幻觉的细节，最后把剩余真实有效的信息拆成可以被镜头表达的画面元素。最终输出 {amount} 个高质量、可检索、去抽象化、且与正文对应新闻场景高度一致的英文搜索词，只输出 JSON 数组。
""".strip()
